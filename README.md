...
| **[2nd-brain-guide](https://github.com/ai4radmed/2nd-brain-guide)** | PARA 운영 방법론·템플릿·빈 vault 골격 (공개 방법론 자산) |
| **2nd-brain** | 개인 knowledge·sources 데이터 (각자 비공개 운영, Syncthing 동기 권장) |
...
- **마운트 최소화**: `~/projects/2nd-brain` 만 컨테이너에 노출. 호스트 `~/`·`/mnt/c`·`/mnt/d`·`/etc` 등 비가시.
...
| 위치 | 경로 |
|---|---|
| 호스트 (.env `SB_DATA`) | `~/projects/2nd-brain` (WSL2 ext4 native, Syncthing 동기) |
| 컨테이너 내부 (고정) | `/workspace/2nd-brain` |

`~/projects/2nd-brain/CLAUDE.md` 의 경로 번역 테이블과 일치시킬 것.
