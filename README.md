...
| 위치 | 경로 | 마운트 모드 |
|---|---|---|
| 호스트 vault (.env `SB_DATA`) | `~/projects/2nd-brain-vault` (WSL2 ext4 native, Syncthing 동기) | `make rw` 시 RW, `make ro` 시 RO |
| 컨테이너 내부 vault (고정) | `/workspace/2nd-brain-vault` | — |
| 호스트 guide (.env `SB_GUIDE`) | `~/projects/2nd-brain-vault-guide` (git 관리, 공개) | 항상 RO |
| 컨테이너 내부 guide (고정) | `/workspace/2nd-brain-vault-guide` | — |

vault 의 `CLAUDE.md` 는 guide 문서들을 `@/workspace/2nd-brain-vault-guide/...` 로 `@`-import 하는 *얇은 layer* 패턴으로 운영합니다. `~/projects/2nd-brain-vault/CLAUDE.md` 의 경로 번역 테이블과 일치시킬 것.
